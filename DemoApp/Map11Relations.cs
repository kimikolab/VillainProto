...
public static class Map11Relations
{
    ...
    private static readonly Dictionary<TraitId, Rule> Rules = new()
    {
        ...
        // 第214期: 名前が「血詠みのアカ」に変わった（仕組みはそのまま）ので、線の1語と説明文の語を「血」に揃えた。
        [TraitId.Ash] = new(Shape.Adjacent, "血が降りかかる",
            "血を抱えたまま倒れると、抱えていた血が隣の味方に等分で降りかかる（そのぶん HP が削れる）",
            LinkSign.Loss, true, "降りかか"),
        // 第214期（禍導のカタ）。**雷を落とすたび**隣の味方に感電が付くので点線（(b) 型）。
        // 感電は起爆するまで残り、その駒が殴られると隣へ放電する——線は「付く」までを描く。
        [TraitId.Thunder] = new(Shape.Adjacent, "感電が付く",
            "雷を落とすたび、この駒に感電が付く（殴られると弾けて、隣の味方へ放電する）",
            LinkSign.Loss, true, "感電"),
        ...
        // 第190期。**隣の味方全員に積む**ので実線。毒そのものは損だが、隣にいる間は反転で薬になるので「両方」。
        // 第191期に手番が「火 → 毒 → 毒」の周期になった（3手番のうち2回）。
        [TraitId.Taint] = new(Shape.Adjacent, "澱みを分ける",
            "3手番のうち2回、この駒に毒が1層積む（隣にいる間は反転で薬になる。離れると本物の毒に戻る）",
            LinkSign.Both, false, "火→毒→毒"),
        // 第191期。周期の火の拍（3手番に1回）で隣の味方全員に着火する。実線・「両方」（澱み分けと同じ理由）。
        [TraitId.Kindle] = new(Shape.Adjacent, "火を分ける",
    ...
    public static readonly Dictionary<TraitId, string> Unresolved = new()
    {
        ...
        // 第212期（ツギの出撃前の板）。列を読むが、相手は「開戦時に一番前の列にいる全員」で隣接ではない
        // ——線の形（`Shape`）に「前列」は無い。開戦時に1回だけ起きる（戦の途中には線の意味が無い）。
        [TraitId.PlankOpening] = "開戦時に一番前の列の全員へ1回だけ（相手は隣接ではなく列で決まる）",
    };
