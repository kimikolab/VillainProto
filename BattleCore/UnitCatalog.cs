...
public static class UnitCatalog
{
    ...
    public static readonly UnitDef Ban = new()
    {
        ...
        //
        // 第127期 段2: その札を**段違い版（`GradeStep`）に差し替えた。**
        // 読む値（`AtkBonus`）も低い側の閾値（`ReaderRule`）も第116期と1ビットも変えていない
        // ——**足したのは上の段（閾値の `GradeTrait.StepFactor` 倍で全体）だけ**である。
        // 第116期が実測した格子（号令 +4 / 縛め +16）は **5 と 20 の2点にしか標本が無い**ので、
        // 上の段は「縛めが乗った行だけ」に立つ。
        Traits = new[] { TraitId.Bulwark, TraitId.GradeStep },
        PlusText = "そのターン動かなかった味方の被ダメージを半減し、外から積まれた力が一定を越えているあいだは自分の一撃が薙ぎに、さらに積まれると全体攻撃になる",
