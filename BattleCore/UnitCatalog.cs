...
public static readonly UnitDef Inverter = Make("inverter", "逆位の祭司", 90, 10, 7, TraitId.Inversion);
    ...
    // 逆位の異端審問官: **第五波の後1に採用した**（2026-08-31・第32期）。審問官1枚と差し替えてある。
    ...
    public static IReadOnlyList<Stage> Stages { get; } = new[]
    {
        ...
        //
        // **後1は逆位の異端審問官（Reverser）。** 審問官と数値・型・断罪が同一で、
        // 盤面ルールを1つ持つ——生きている間、**両陣営の行動順が速さ昇順になる**。
        // 狙いは「総合力」しか問うていなかったこの波に、**先に動けることという別の軸を
        // 1本足す**こと。第31期の測定でこの波は固有の勝者0・固有の敗者1まで痩せ、
        // 第三波との相関 +0.56 が盤全体で最大の波間重複になっていた
        // ——第三波と第五波は速さの多重集合が {14,11,7,7,5} と {14,12,10,7,5} で、
        // **攻20・速14 の勇者候補が前3 から最初に動く開幕**まで共有している。
        // 反転はその開幕だけを崩す（第三波には触らない）。
        // 中央ではなく後1 に置いた理由は Reverser の宣言に全文。
        // 教えること:「速さを数えて編成を組んだなら、その向きが逆になる盤面がある」。
        new Stage("第五波 / 異端審問団",
            Formation.Build(front1: Axeman2, front3: Hero2, center: Knight2, back1: Reverser, back3: Lancer))
    };
