...
public static class UnitCatalog
{
    ...
    public static readonly UnitDef Mudo = new()
    {
        ...
        PlusText = "殴られた回数を溜め、3回で閾を越えた瞬間その場で割り込み、溜めた回数だけ殴り返す（1発ごとに少し回復する）。殴ってきた相手には呪いが付き、呪われた者どうしは単体の一撃の痛みを半分ずつ分け合う",
        MinusText = "暴れると泥が飛び散り、隣の味方の得物が鈍る（攻撃力低下）。素の攻撃力はほぼ無く、倒れると溜めた怒りは不発。味方の刃に殴られると、その味方にも呪いが付く",
    ...
    public static readonly UnitDef Ban = new()
    {
        ...
        PlusText = "手番で踏みしめて守りを固める。殴られても1層積もる（1層ごとに被ダメージ -10%・5層まで）。隣の味方に及ぶ範囲攻撃を代わりに受け止める（受け止めた分は半分になる）",
    ...
    public static readonly UnitDef Sasa = new()
    {
        ...
        // 第213期: 規定を Z2（`Brace` だけ・第211期の姿）に戻した。第212期の Z3（`BraceArmored`）は受け切った一撃でも弾くので、
        // X 字の `継ぎ当て×分散回復` でセロが弾かれて倒れた。上限だけを破片より前に掛ける版（W1 ＝ `BraceCapFirst` ／
        // W2 ＝ ＋ `BraceHeldDeliver`）は札の差し替えで並べる（design/PHASE213_BRACE_CAP.md）。
