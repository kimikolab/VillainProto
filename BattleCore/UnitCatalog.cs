...
public static class UnitCatalog
{
    ...
    /// <summary>
    /// 追い打ちのハギ。<b>第129期に <c>MaxHp</c> を 62 → 82 にした。</b>
    ///
    /// <para><b>触ったのは `MaxHp` 1つだけ</b>（攻撃力・速さ・特性・攻撃型・席・<c>Presets</c> は
    /// 1文字も触っていない）。理由は「弱いから強くした」ではなく
    /// <b>「起動する前に落ちるので、機構が一度も実演されない」</b>——第126期の実測で
    /// 生存 2.84T・与ダメ <b>0.8/戦（推奨6行 30 席で最低）</b>、つまり<b>置物のまま終わる。</b></para>
    ///
    /// <para><b>ロスターで唯一、延命すると「発火の密度」（発火 ÷ 生存T）が<u>上がる</u>駒</b>
    /// （第129期 段2・器具 <see cref="TraitId.Undying"/> で 0.463 → 0.508 の <b>×1.10</b>。
    /// 他の4枚はソラ ×0.95・セロ ×0.74・ガルド ×0.65・トメ ×0.55 と<b>全部下がる</b>）。
    /// <b>追い打ちの鍵は「味方の撃破」で、撃破は敵が削れた戦闘後半に集まる</b>ので、
    /// 早く落ちる駒は鍵が配られる時間帯に居合わせない——<b>買った時間が時間以上の仕事になる。</b></para>
    ///
    /// <para><b>第126期の線（+1.0T）には届かない。</b> `MaxHp` を +30 しても買える時間は
    /// +0.52T で、<b>HP では +1.0T に到達できない</b>（第129期 段3 の掃引）。
    /// だから量は時間ではなく<b>密度と代金</b>で選んだ——+20 で 発火 ×1.18・与ダメ ×1.15・
    /// 密度 ×1.06 に対し、<b>情報セルは 165 のまま1つも減らず</b>、拒否権は 0 行、
    /// 主判定19行の第五波は +1.0pt（歯止め 33.2% との余裕 +9.1 → +10.1pt）。
    /// +30 にすると `追撃×毒` 第3波が +65.5pt 跳ねるので採らなかった。</para>
    ///
    /// <para><b>説明文は1文字も変えていない</b>（能力値の変更は文に出ない・指示書 §3-3）。</para>
    /// </summary>
