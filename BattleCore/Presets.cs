...
public static class Presets
{
    ...
    public static (string Name, Formation F)[] Compare => new (string, Formation)[]
    {
        ...
        //
        // **鱗のウロを同席させてある**（後1）。ウロは破片を**二値で読む**（`Armor > 0` で攻撃型が上がる）唯一の駒で、
        // ガレは**量で読む**。第138期の主産物はここで、**同じ在庫を取り合っても符号が逆に出る**
        // ——ウロの纏い率は 42.6% → 35.1%・生存Tは 8.56 → 5.21T と読み手としては明確に痩せるのに、
        // **その台の帰属が4台で最大（+36.4pt）**になった（第137期「稼働率↑・勝率↓」の完全な裏返し）。
        //
        // **5枚は第138期 段4 の台A そのまま。** 42 組を走査して「4台すべてが 40〜95% に入るのは
        // ドルガ × ガルドの2通りだけ」と結論した組で、**床にも天井にも落ちないことが既に測ってある**
        // （V0 ＝ ガレを素体に落とした版で 48.5%・M=3 で 82.1%）。
        // **ガルドは破片を1点も書かず読まない**ので礫の帰属には入らないが、**壁として台を成立させているのはこの1枚**である。
        //
        // **`溜め (ガン×ドルガ×カド)` と差し替えた**（第139期・行数は 61 のまま）。
        // 落とす行は第111期の選定規則（`ledger phase0`）に選ばせた——主判定19行を外した 42 行のうち
        // 線（r ≥ 0.95 かつ max|Δ| ≤ 10pt）を満たす 10 行で、**「r が最大」でも「情報セル最少」でも同じ行**に落ちる。
        // `溜め` は `docs/balance.md` の実値で **`耐久 (ガルド×ノノ)` と全5波が1ビットも違わなかった**
        // （100.0 / 100.0 / 100.0 / 100.0 / 90.5・r = +1.0000・max|Δ| = 0.0・情報セル 1）。
        //
        // **指示書の推奨2つはどちらも採れなかった**——`刻み×澱み (ノミ×ミオ)` は
        // `woundIgnite` の**唯一の観測点**（`ledger unique` を HEAD で実測。落とすと規則が盤面から見えなくなる）、
        // `刻み×抉り (ノミ×エグ)` は**主判定19行のメンバー**（落とすと歯止めの分母が動く）。
        // **行を増やす道も閉じている**——`305` の決め打ちが3箇所ある（`traits` / `pairs` / `wound` / `wcost`）。
        ...
        // この行は第2波 −22.0pt で拒否権3 に触れたまま残っていた（第107期「機構が変わった行の
        // 席は寿命が切れる」の、駒そのものを入れ替えた版）。`docs/reseat.md` の粗順4
        ...
        // 傷軸・第6弾（第108期）。**澱みのミオ**＝傷を毒に変える読み手。
        // 第39期からここに居た `刻み×縫い (ノミ×ハリ)` の差し替えで、**土台の3枚は動かさない**
        // （ゴルム／ドルガ／ヴェル）——読み手1枚だけを替えるという第37期以来の作法をそのまま継いだ。
        //
        // **ハリはロスターから外れた**（第108期 (T2)）。切った根拠は4期ぶんある——
        // 第83期の3分の器具で唯一「切れる」判定、第85期の両側読み（律速は振り 2.15 回/戦）、
        // 第106期の再行動が 0.00 回/戦（再行動は傷の**書き手**にしか渡らない）、
        // 第107期の発火口の移設（傷の書き込みの 90.5% が巻き込み則＝味方側なので敵の傷を塞ぐ機会が消える）。
        // **3期にわたって律速が同じ場所（振り回数）で、発火口を移しても解けなかった。**
        // 定義（`SutureTrait` / `SutureRule` / `SutureFireRule` / `UnitCatalog.Hari`）は残置してある。
        //
        // **読み手にミオを採った理由は「重複しないこと」1つ**（第108期 Phase 0）。
        // 傷の読み手は5枚（エグ・ナタ・ハリ・ミオ・ノノ）だが、ハリの席に
        // **エグを入れると `刻み×抉り` と、ナタを入れると `刻み×断ち` と5枚組が完全に一致する**
        // （席が違うだけの冗長な行になる。第39期が `裂き×縫い` を落としたのと同じ理由）。
        // ノノの繕いは患者が必ず同陣営（`Where.Ally`）なので、**ノミが敵に刻んだ傷を1つも読まない**
        // ——「ノミが刻んで、読み手が読む」というこの行の意図が消える。
        // **敵の傷を読み、かつ重複しないのはミオだけ。**
        //
        // ミオは傷を**毒**に変える。`IgniteRule`（第89期・既定 on）が「傷を持ち毒を持たない敵」に
        // 毒 1 を置き、滲み則（第90期・`SoakRule.Poison`）が「傷を持つ相手には毒が +1 深く入る」を足し、
        // 澱み本体（+4層）がそれを濃くする。**傷 → 毒 の3段が1行に乗る。**
        // 第87期は「ミオを含む4行が全部グザ（瘴気）同席で、キリ／ノミとの交わりは 0 行」なので
        // 着火が1回も起きない（毎ターン敵全体に毒が撒かれていて「毒を持たない敵」が存在しない）と
        // 測っていた。**この行はグザを含まない初めてのミオの行で、その空白を埋める。**
        //
        // 代金は体——ハリ（HP54・攻10・普通に殴る）がミオ（HP42・攻2・`Actions=[Skill]` で振らない）に
        // 替わるので、出力はドルガ（攻38・薙ぎ）とノミ（攻10・刻み）の2枚だけになる。
        // **席は測り直した**（第108期・`hold2 seats` ＝ 第107期の器具。粗探索 seed 0..49 →
        // 作法1・2 の帯 0..199 → 追試 200..599）。仮置き（ハリの席にミオを置いただけ）は **19 位 45.5%** で、
        // 狙を満たし情報セルを 2 以上に保つ最上位が **70.6%**——差 **+25.1pt**（情報セルは 3 のまま）。
        // **中央（次数4）が 刻みのノミ → のろまの巨兵ドルガ**に替わり、**ミオが前1へ出る。**
        // ミオは隣接も列も読まない（澱みは敵の毒と傷しか見ない）ので前に出しても機構は1つも損なわれず、
        // 中央の次数4は薙ぎのドルガが取るほうが効く。第107期の
        // 「**機構が変わった行の席は寿命が切れる**」がそのまま当たった——
        // 旧席は「傷持ちがいなくても普通に殴るハリをどこに置くか」の答えだった（第39期）。
        ("刻み×澱み (ノミ×ミオ)", Formation.Build(front1: UnitCatalog.Mio, front3: UnitCatalog.Golm,
                                            center: UnitCatalog.Dolga, back1: UnitCatalog.Vel,
                                            back3: UnitCatalog.Nomi)),
        ...
        //
        // **第186期 追補で席を替えた**（前1 カド／前3 ボルグ／後3 ソラ → 前1 ソラ／前3 カド／後3 ボルグ）。
        // `reseat` の1位（帯A 85.0%・逸らしを入れた後）を、突きも入れた盤面で**選定に使っていない seed 200..599** で追試して
        // 82.4 → 97.7%（**+15.3pt**・閾値 +5.0pt）。情報セルは帯A で 2 → 2（`sora186 seat`）。
        // **この席ではヒサの隣がソラ（96）とグザ（58）になるので、ヒサはカドではなくソラを指す**
        // ——上に書いた狙「ヒサの隣接最大HPは必ずカド＝第49期の残件Aの再現条件」は**この行では捨てた**
        // （ポンの判断で席替えを採った）。代わりにソラ×ヒサの「1/4 だけ受ける」がこの行に立つ（重なり 0.07 → 3.72 回/戦）。
        ("逸らし (ソラ×カド)", Formation.Build(front1: UnitCatalog.Sora, front3: UnitCatalog.Kado,
        ...
        ("火選り無風型 (ヒヨ×カド)", Formation.Build(front1: UnitCatalog.Kado, front3: UnitCatalog.Hiyo,
                                              ...
                                              back3: UnitCatalog.Borg)),
        // **第111期に `死軸×ホタ (ゾト×熾)` をここから外し、下の尾灯の行に差し替えた。**
        // 落とした根拠は3つで、どれも結果を見る前に固定した規則から出ている（design/PHASE111_LEDGER.md §2）:
        //   (a) 主判定19行に入っていない ／ (b) その行でしか観測できない機構が無い
        //       （`blaze` を切ると 9 行が動く。**唯一の観測点ではない**）／
        //   (c) **61 行でいちばん冗長**——`後衛特化+後備え` との r が **+1.0000**・max|Δ| が **0.5pt**。
        // しかも情報セルが 1 しか無く、**その 1 は第89期 (P2) の席の差し替えで 3 → 1 に落ちたもの**
        // （100/100/100/100/94.0）。**第89期は追試の帯 200..599 で情報セル 2 を確かめて採ったが、
        // `compare` が測る帯 0..199 では 1 になる**（第四波が 99.8% 対 100.0%）
        // ——**情報セルは `compare` が使う帯で数えること**（第77期「天井の際で不安定」の実例）。
        //
        // 第59期。破裂の着火（`BlazeRule.Both`）が入ったので**ボルグ抜きで火が回る**。
        // 試験行は `死の連鎖 (リィカ軸)` の**ムグ1枠だけ**を差し替えた形——1枠差にすると
        // 既存行がそのまま対照になる。**`reseat` は出発点が 15位/120** で、
        ...
        // **第111期。尾灯のトモを初めて盤面に出した行**（第108期に `UnitCatalog.All` へ入れてから3期ぶり）。
        // トモが書くのは**強化1本だけ**（`TraitEntryMap.Supplies[Taillight]` = 強化（Ally））で、
        // 譲渡は手番の通貨を1つも書かない——**新しい交差ではなく、強化軸の新しい書き手**なので
        // 行き先は交差帯ではなく `compare` の側（交差帯には `強化×手番 (ガン×ドルガ×バン)` が既にある）。
        //
        // 5枚組は**第110期で唯一まともに測れた台**をそのまま採った（帰属 V1 +46.0pt）。要は
        // **灯の対象がドルガに一意に決まる**こと——トモを除いて最も遅いのは速4のガルドだが
        // `Stoic` で灯の候補から外れるので、次に遅いドルガ（速6）に決まる（2番手はボルグ・ソラの速8）。
        // ドルガは自分の手番で振る駒なので、譲られた手番が潰れない（第109期の「`CanAct` が偽の駒に
        // 手番を譲っても何も起きない」を避けてある）。**`AtkBonus` を他人に書く駒は1枚も入れていない**
        // ので、灯の到達点に別経路が混ざらない。
        //
        // 席は**第114期に差し替えた**（後1 と 後3 を入れ替えただけ。**5枚組は1枚も動かしていない**）。
        // 灯の濾しを W2（`LitFilter.ActingNow`）に採ったぶんこの行が強くなり、第四波が 74.0 → 98.0% と
        // 天井へ寄って**情報セルが 4 → 3 に落ちた**ため——**情報を減らす変更は席の再判定と対にする**（第107期）。
        // 120 通りを W2 で測り直し、**狙（ガルドが前列）○ かつ情報セル 2 以上**の 17 通りのうち
        // **情報セルが最大（4）の段の最上位**を採った（追順3位・平均 91.2 → 87.6% ＝ −3.6pt で
        // 採否閾値 5.0pt の内側。帯B でも −2.4pt）。**勝率の1位ではなく「測れる席」を採っている**（第50・64期）。
