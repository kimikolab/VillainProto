...
public static class Presets
{
    ...
    public static (string Name, Formation F)[] Compare => new (string, Formation)[]
    {
        ...
        // ので、灯の到達点に別経路が混ざらない。
        //
        // 席は 120 通りを情報セルつきで測り直して**第110期の並びを据え置いた**——狙（ガルドが前列）を
        // 満たし情報セルを 2 以上に保つ最上位は追順1位だが、出発点との差が **+0.8pt** で
        // 採否閾値 5.0pt に届かない（第46期の作法2）。**出発点は粗探索1位・情報セルは 4 で候補中の最大。**
        ("灯×薙ぎ (トモ×ドルガ)", Formation.Build(front1: UnitCatalog.Dolga, front3: UnitCatalog.Gald,
                                              center: UnitCatalog.Sora, back1: UnitCatalog.Tomo,
                                              back3: UnitCatalog.Borg))
    };
