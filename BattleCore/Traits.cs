...
public enum TraitId
{
    ...
    Avenge,      // 仇討ち: 標的にされた味方が殴られると割り込んで刺し返す。自分が殴られると怯む
                 //（同上）
    Rend,        // 裂き: 攻撃した相手に傷を刻む。身も薄く、受けるダメージが1.5倍
...
///
/// **1つのルールの表と裏**（置き去り・責め苦・仇討ちと同型）: 刃を薄く削ぎ続けたから斬り口が残る。
/// 薄く削いだのは刃だけではないので、自分の身も薄い。プラスとマイナスが同じ一文から出る。
///
/// **刻む数はダメージ量に依存しない。** ドルガの38もキリの12も等価に傷1。
/// 量に比例させた瞬間に「強い駒がもっと強くなる」乗算の道に入る（README「増幅は必ず加算にする」の
/// 物理版で、比例させるなら結局は与ダメージをもう一度読むだけになる）。
/// 「原因ではなく結果で解決する」——誰が刻んだ傷でも読み手は同じように読む。
///
/// <para><b>撤回したマイナス（第28期の初版 → 第29期）:「与えるダメージは常に1」。</b>
/// <c>ModifyAttack</c> を全経路で1に潰す形で、可変コスト型としては綺麗だった
/// ——引数を読まない限り上流に何が乗っても動かないので、床も天井も要らなかった。
/// **撤回の理由は機構ではなく予算。** 傷軸は供給（キリ）と変換（エグ）を別の駒に分離した
/// 設計で、そこへ「単独での出力が 1/ターン」を課すと**1枠まるごと出力ゼロ**になる。
/// 毒軸が5枠で成立するのはスィド・グザが積みながら同時に殴っているから（供給と出力が
/// 同じ1手に同居している）で、傷軸はそれを持たない。第28期の第2編成は
/// <c>reseat</c> の120通りが 20.0〜20.3% に横並びになり、**配置で1ビットも動かない盤面**に
/// なった。ザンの「1ターンに1回」撤去と同じ構図——**設計として正しい制約が、
/// 5枠の予算制約下では軸を殺していた。**</para>
///
/// <para><b>差し替えた形: 出力ではなく配置に代金を移す。</b> 攻撃12がそのまま出るので
/// キリは普通に殴れる駒になり、マイナスは<see cref="ModifyIncomingDamage"/>（受けるダメージ
/// 1.5倍）へ移る。速さ12＝前に出やすく HP48＝薄い駒が1.5倍で殴られるので、
/// 「刻み役を守る」という配置の判断が生まれ、庇う（ガルド）・後備え（セッキ）・
/// 破片（ヒビ）と噛む。傷は「量に依存しない1発1つ」のままなので、
/// 第28期に実測した線形性はそのまま保たれる。</para>
///
/// <para><b>採らなかった案。</b>
...
public sealed class RendTrait : Trait
{
    ...
    /// <summary>
    /// 薄皮。<b>自分が受けるダメージは常に 1.5 倍。</b>
    ///
    /// <para><see cref="FrailTrait"/>（脆弱）と**完全に同じ式**。新しい機構はゼロで、
    /// 端数の丸め（<c>dmg + dmg / 2</c> の整数除算＝切り捨て）も揃えてある
    /// ——同じ意味のマイナスが2つの丸め方を持つと、どちらが正なのかが後で決まらない。</para>
    ///
    /// <para>ここは <c>ApplyDamage</c> の中盤（惨禍・据え・散開・萎縮より前、
    /// 肩代わりより前）で効く。**軛（<c>YokeTrait.Cap</c>）は最後に切る**ので、
    /// 上限のある波では 1.5 倍が上限に飲まれることがある——それは意図どおりで、
    /// 「1発の重さ」に課金する盤面ルールの側の帰結。</para>
    /// </summary>
    public override int ModifyIncomingDamage(UnitState self, int dmg) => dmg + dmg / 2;
