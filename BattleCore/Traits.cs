...
public enum TraitId
{
    ...
    // --- 第153期で足した札（**回復に「時間」の次元を入れる**。`UnitCatalog.All` には入れない） ---
    // **2枚に分けてあるのは器具の要件**（第74期に 裂き → `Rend` + `ThinBlade` を切り出したのと同じ作法）。
    // 1枚に畳むと `checkup` の `yP`（マイナスを外した版）が作れず、(ii)＝分離できない駒になる。
    // **挙動は畳んだ版と1ビットも変わらない。**
    Ward,       // 預かり: 味方が受けた傷の一部を預かりとして積み、`WardRule.Return` の条件で本人へ返す
                // ——**積むときは `ctx.Heal` を通らず、返すときだけ通る**ので、
...
public sealed class GuardianTrait : RedirectGainTrait
{
    ...
    ///
    /// <para><b>第136期 段0: ガルドの一文から落とした（実装は残置・既定 <c>false</c> のまま）。</b>
    /// 理由は3つ——(1) 実測 <b>0.00 回/戦</b>（`docs/harm.md` 表G・第135期）、
    /// (2) 第122期に既定オフ、(3) <b>巻き込み則（<see cref="SpillWoundRule"/>）も同時に降りたので、
    /// 規則を戻しても 0 のまま</b>——隣接する味方に傷が載る経路そのものが盤面から消えている
    /// （敵に傷を書く裂き・刻みは味方には書かない）。直すには巻き込み則に触ることになるので、
    /// この期では扱わない。<b>コードを消さない</b>のは診断 <c>gather</c> / <c>wound2</c> が
    /// この計数（<c>GatherGuards</c> / <c>GatherHadDonor</c>）をそのまま読むため。</para>
    ///
...
public sealed class ShatterTrait : Trait
{
    ...
    // ヒビのHPという有限プールがそのまま天井になるため。
    // 上限20/12/30 を振っても差が出なかったので、効いていない定数を残すより外した。
    // 変換率を50まで上げると初めて効き始める。
    //
    // **第137期に数字を書き換えた。** ここには「実測で1戦の被弾110、配れる破片は
    // 味方1体あたり30弱」と書いてあったが、**現行の盤面では成り立たない**
    // ——`shard phase0` の実測（ヒビを含む 5 行 × 第2〜5波 × seed 0..199）で
    // **被弾 61.8 / 味方1体あたり 8.4**。どちらも当時の3分の1以下で、
    // ヒビは **94.2% の戦で死に、平均 3.24T しか生きない**。
    // 第122期「規則を降ろすと説明文が静かに嘘になる」の**数値版**で、
    // こちらは規則ではなく**盤面が動いたので数字が古くなった**。
    // **実測値を書くときは測った期を添えること。**
...
/// 身構えの規則（第143期）。
///
/// <para><b><c>Cap = 0</c> は上限なし。</b> 切り落としが1点も出ないので <see cref="Refuse"/> も同時に不活性になり、
/// <c>(0, false, false)</c> と <c>(0, true, false)</c> は 1 セルも違わない（段A の対照・自己検査 (a)）。</para>
///
/// <para><b><see cref="Stagger"/> だけは <see cref="Cap"/> に依らない。</b>
/// 転倒は<b>上限ではなく弾きに付いている</b>ので、<c>Cap = 0</c> でも突き飛ばした味方は転ぶ
/// ——<c>(0, false, true)</c> は盤面を動かす。<b>「上限を 0 にすれば全部止まる」ではない。</b></para>
/// </summary>
/// <param name="Cap">
