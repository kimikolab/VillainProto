...
func capture():
    ...
    scene.call("DropUnit", 3, "mudo")
    scene.call("DropUnit", 4, "gald")
    scene.call("OnSetupSlotClicked", 2)
    await create_timer(0.5).timeout
    await RenderingServer.frame_post_draw
    var result = root.get_texture().get_image().save_png("res://../design/art/aka/selection-game-check.png")
    if result != OK:
        quit(1)
        return
    ...
    await create_timer(0.5).timeout
    await RenderingServer.frame_post_draw
    result = root.get_texture().get_image().save_png("res://../design/art/aka/battle-game-check.png")
