...
**受け流しは第136期 段2 に本採用した**（`ParryRule.Default` ＝ `Uses 4 / Any / Relay true`・保持者はガルド）。
...
| 第136期 | ガルドを壁にした（**3段で盤面を動かした**）。庇い 50 → 100%・受け流し（在庫 4・構えで毎ターン戻す・見返りは攻撃力から在庫へ）・中継。**ガルドは 3.48 → 10.97T・被弾の 91% を受け流す**。情報セル 165 → 122。**無効化は被弾を入力にする札を飢えさせ、殴らないガルドは膠着を作る**（`反撃` 第3波 100 → 0） |
