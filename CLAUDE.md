...
現状値は README「波に『1発の上限』を置いたら、第四波が課税する資源を変えた」に貼ってある
（第25期・軛の後の世代。渇きの節はその1つ前、「波の分離度を測る物差しを作った」の表は
**さらに前の値**）。波を作り直すときは、この表の前後比較で採否を決める。
**計測器と測定対象を同時に動かさない。**

第25期の後の波の並びは 平均 100 / 85.8 / 72.5 / 61.8 / 59.8 で**単調に落ちる**。
...
- **盤面ルール（`Inversion` / `Drought` / `Yoke`）だけは例外で、判定が engine 側にある。** どれも
