...
所要は全編成でおおむね 10〜30秒、`bridge` だけ 30秒前後。
    ...
    dotnet run --project BattleSim -c Release 0 kindle [絞り込み] # 焚き付け: 発火・空振り・燃体/非燃体・熾火への配分・死蔵・火の粉の符号の反転（第58期）
    dotnet run --project BattleSim -c Release 0 kindle phase0    # 実装前の地図（**熾火の乗算監査**・在庫・試験行の選定）
    dotnet run --project BattleSim -c Release 0 kindle sweep     # Gain / Loss の掃引（V0〜V4・素体の対照つき）
    dotnet run --project BattleSim -c Release 0 kindle seats     # 席の分散（seats2 の写し）＋ **reseat 120通りの帯の形**
    dotnet run --project BattleSim -c Release 0 kindle confirm   # 配置の追試（seed 200..599）。**発火／空振り／燃体／非燃体の列つき**
    dotnet run --project BattleSim -c Release 0 kindle alt       # 機構の帰属と火の粉の符号を別 seed 帯で追試
...
**残り枠は 1**（第58期時点。`UnitCatalog.All` は 51 体＝ソラ・カリ・トメ・フイを含む。上限は 52）。
**うち1枠はオゴという素材が既にある**（第46期に棄却して残置しただけで `All` には載っていない）。
**第48期の「残り 5 / 分母 47」は古い**——第50期にソラ、第52期にカリ、第53期にトメが入った。

**通貨の厚みは駒で数える。CLAUDE.md の「毒 6書き/4読み」等は呼び出し箇所の数で、単位が違う。**
駒で数えると 毒 5/4・標 **3/2**（第52期にカリが2枚目の書き手／第53期にトメが2枚目の読み手）・痺 5(+敵3)/1・燃 **2/2**（第58期にフイが2枚目の読み手／**第59期にゾトが2枚目の書き手**）・IdleTurn 0/3・破片 3/1・傷 4/4・
...
燃は第58期にフイ（2枚目の読み手）・**第59期にゾト（2枚目の書き手）**が入ったので、
**相互唯一のペアは1組も残っていない。**
...
**第58期に焚き付け（フイ）が2枚目の読み手になり、18 セルのうち 2 つ**
...
現状値は **61編成**（第59期。破裂の着火を採用し試験行2本を足した後）で
...
（第二波の固有の敗者に `焚き付け無風型 (フイ×カド)` が加わって 7 → 8 だけ）。
...
**2番目の波が3番目より難しい**。**第五波の平均は歯止め 40.0 を 2.6pt 割っている**（61編成で 37.4・59編成なら 35.9）が、
これは行を足した組成効果の累積で、波そのものは第40期の 40.5（44編成）から動いていない
（第41期 39.6/45行 → 第47期 37.8/48行 → 38.6/50行 → 第53期 37.0/56行 → 第58期 35.9/59行
→ **第59期 37.4/61行**。**既存行のセルは第58期まで各世代で 0 件の不変**だったが、
**第59期は盤面ルールの採用期なので既存7行が動いた**）。
**第59期に「主判定の固定行集合の上で測る」への移行案を出してある**（design/PHASE59_BLAZE.md §9-4）
——実測で **主判定17行なら 41.9 / 提案20行なら 40.0** で、全行平均が低いのは分母の話である。
**下回り幅は第41期の 0.4pt から積み上がっている。**
...
- **弱体化（`AtkBonus` を負にすること）の窓口は `BattleContext.Dull` の1箇所**（第42期）。
  ...
  **渡しの転嫁**／**焚き付けの鈍り**。転嫁だけが宛先が敵側で、窓口の中から窓口を呼ぶ。
  焚き付け＝第58期は**状態異常を条件に宛先を選ぶ初めての弱体経路**）。
...
- **攻撃力を上げる唯一の窓口は `BattleContext.Whet` の1箇所**（第56期）。
  **`AtkBonus` を直接足さないこと。** `Dull`（鈍らせる）の対義で、通るのは**他者強化の7経路だけ**
  ...
  移り木（シオ）／**吐き戻し（ゴルム・engine 側）**／**焚き付け（フイ・第58期。
...
- **「機構として起こりうる」と「盤面で起きている」は別**（第56期）。第52期の
  ...
  **吐き戻し1本だけ**・0.22 量/戦、符号が正へ渡った回数は **0.002 回/戦**でほぼ安全）。
  窓口はこれを常時見せるためにある。
- **燃焼を読んで強化・弱体へ変換する経路は焚き付け（`KindleTrait`・フイ）が初めて**（第58期）。
  第57期の表E（9通貨 × 双方向）は 18 セルすべて 0 で、**燃焼は盤面のどこにも繋がっていなかった。**
  **engine には規則も窓口も1つも足していない**——`Whet`（第56期）と `Dull`（第42/43期）に
  `WhetRoute.Kindle` / `DullRoute.Kindle` を**末尾に1つずつ**足しただけ。
  `OnTurnStart` の1回の発火で **プラス＝燃えている味方全員（自分を除く・位置を問わない）に
...
- **2本のノブを持つ機構では、「一方を動かすと他方の計数が動くか」を掃引の表で確かめる**（第58期）。
  `KindleRule` の採用値は **`Gain = 4` / `Loss = 2`**（探索段階の初期値 2/2 から動かした。
