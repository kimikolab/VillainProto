...
所要は全編成でおおむね 10〜30秒、`bridge` だけ 30秒前後。
    ...
    dotnet run --project BattleSim -c Release 0 creak          # 軋みが響く: AtkBonus >= 閾値 で単体→薙ぎ（第66期・**採用しなかった**）
    dotnet run --project BattleSim -c Release 0 creak phase0   # 実装前の地図（割り込みの経路 / ヨミの AtkBonus の分布と出どころ / Q6 の実効レンジ）
...
`DisplacedTrait.ModifyPattern` は、3行すべてで帰属 **+17.1 / +10.8 / +13.5pt**（両 seed 帯で幅 1.1pt 以内・
