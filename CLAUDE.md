...
- **強化の2枚目の読み手が積み過ぎ（`OverloadTrait`・第115期）。既定は不活性で、盤面にはまだ載っていない。**
  ...
  **搭載（`UnitCatalog` への札の追加と既定の切り替え）は席の再判定と対にするので次の期**
  ——経緯は design/PHASE115_READER.md。**読み手は 2 枚になったが、量を読む駒と事象を読む駒は 0 枚のまま。**
...
**第115期は盤面を1ビットも動かしていない**——強化の2枚目の読み手（積み過ぎ・`ReaderRule`）を
実装して測り、**採る判定は出したが既定は不活性のまま残置**した（搭載は席の再判定と対にするので次の期）。
`compare` 305 セルは 0 件で、下の値はそのまま生きている。

現状値は **61編成**（第114期。**灯の濾しを W2 に採り、`灯×薙ぎ` の席を差し替えた後**）で
...
| 弱体・強化（なまり・呪詛・萎縮・突き返し・引き受け・転嫁・横流し・火選り・呪い則・灯の濾し・積み過ぎ） | 第 41, 42, 43, 44, 56 / 62, 63, 64, 65, 66, 67, 68, 77 / 95 / 106, 113, 114, 115 期 |
