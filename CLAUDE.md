...
現状値は README「波に『回復禁止』を置いたら、第三波が初めて分離した」に貼ってある
（Phase 2b の後の世代。「波の分離度を測る物差しを作った」の表は**それより前の値**）。
波を作り直すときは、この表の前後比較で採否を決める。**計測器と測定対象を同時に動かさない。**
...
- **盤面ルール（`Inversion` / `Drought`）だけは例外で、判定が engine 側にある。** どちらも
