...
**第135期も盤面を1ビットも動かしていない**（段1 は計数だけ・段2 の `ParryRule.Uses` は既定 0 で残置）。
**指示書が引いている第132〜134期の文書は `design/` に存在しない**——実体は `PHASE131_ADVANCE.md` までで、
下の期別表も第131期で終わっている。**現状値は第131期のまま。**

現状値は **61編成**（**第131期は盤面を1ビットも動かしていない**ので数字は第130期のまま）で
...
所要は全編成でおおむね 10〜30秒、`bridge` だけ 30秒前後。
    ...
    dotnet run --project BattleSim -c Release 0 checkup check    # 受け入れ基準: `compare` 305 セル ＋ 第119期の自己検査 (a)〜(d)
                                                                 # **第128期から止まっている**（`GradeStep` に分類が無い。第131期に判明）
...
| 毒・燃焼（瘴気・澱み・着火・滲み則・火の粉・熾火・破裂・**配布**） | 第 57, 58, 59, 60 / 61, 87, 89, 90, 91, 96 / **130** 期 |
...
| 攻撃型と段（格上げ・軋み・積み過ぎ・鱗・熾火・後衛特化） | 第 47, 66, 67, 77 / 115, 116, **127, 128** 期 |
