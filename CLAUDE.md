...
駒で数えると 毒 5/4・標 **3/2**（第52期にカリが2枚目の書き手／第53期にトメが2枚目の読み手）・痺 5(+敵3)/1・燃 **1/2**（第58期にフイが2枚目の読み手）・IdleTurn 0/3・破片 3/1・傷 4/4・
...
燃は**第58期にフイ（2枚目の読み手）**が入ったので、**相互唯一のペアは1組も残っていない。**
現在も唯一なのは **ボルグ（燃の唯一の書き手）／シガ（痺の唯一の読み手）／ウロ（破片の唯一の読み手）の3体。**
ホタ・ヒサ・ザンは唯一ではなくなったが、**ホタは燃の 52.3pt の帰属を単独で持つ**ので値段は別の話。
...
現状値は **59編成**（第58期。焚き付け3行を足した後）で 100 / 66.9 / 73.6 / 57.0 / **35.9**
...
**編成の行数が世代で違う**（38 → 42 → 45 → 48 → 50 → 52 → 54 → 56 → **59**）ので、波を作り直すときは
