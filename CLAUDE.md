...
**採用後の `reseat` でこの行の現行席は 15 位（5波平均 30.2%）に落ちた**——1位 38.7%（+8.5pt）／狙（ガルド前列）を満たす最上位は 4 位 35.8%（+5.6pt）で、**第46期の作法では `confirm` にかける行。第106期では動かしていない。**
...
現状値は **61編成**（第106期。ノノの代金半額とササの弾きを採用した後）で
**100 / 69.1 / 75.6 / 64.7 / 42.9**（**主判定19行では 100 / 70.9 / 71.6 / 69.3 / 41.1**）
——**歯止め 33.2% との余裕は +7.9pt。**
