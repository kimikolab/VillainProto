...
**新しい則を足すとき**: `design/RULES_*.md` の末尾に次の ID（`R266` から）で本文を書き、
...
本体は `DemoApp/Map11.cs`（進行の規則）と `Map11Main.cs`（画面。判定は持たない）。
    ...
    Godot.exe --path DemoApp --quit-after 400 res://Map11Main.tscn -- "--map11-capture=out.png[,detail|encounter|pick]"
...
| 表示と「見る」（`GodotApp` / `DemoApp` / 台本の窓口 / 盲点表 / 戦績パネル / 書き手 / 手番の外 / **遊べるマップ** / **情報の層** / **配置の図と封じ**） | 第 97, 98, 99 / **123, 124, 125** / **169, 170, 171** 期 |
