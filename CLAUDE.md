...
所要は全編成でおおむね 10〜30秒、`bridge` だけ 30秒前後。
    ...
    dotnet run --project BattleSim -c Release 0 funnel [モード] # 横流し: 強化の行き先を編成が選ぶ（第62期・**採用しなかった**）。V1 遅い / V2 速い / 素体
    dotnet run --project BattleSim -c Release 0 funnel phase0    # 実装前の地図（`Whet` の7経路・**速さ表**・宛先の机上計算・歯止めの自己検査）。戦闘0回
    dotnet run --project BattleSim -c Release 0 funnel sweep     # 選択子の対照（V1 / V2 / 素体）だけ
    dotnet run --project BattleSim -c Release 0 funnel seats     # `reseat` 120通りの帯（**素体の帯を併記**）＋ 席ごとの帰属（**素体との差**）
    dotnet run --project BattleSim -c Release 0 funnel confirm   # 配置の追試（seed 200..599）。**横流し／死蔵／並びの列つき**
    dotnet run --project BattleSim -c Release 0 funnel alt       # 帰属の符号を別 seed 帯（200..599）で追試
...
**残り枠は 1**（第62期時点も変わらず。`UnitCatalog.All` は 51 体＝ソラ・カリ・トメ・ヒヨを含む。上限は 52）。
**その1枠に充てられる素材は3枚ある**——オゴ（第46期）・ゴウ（第49期）・**ヌキ（第62期）**。
...
- **強化の「行き先」を書き換える経路は横流し（`FunnelTrait`・ヌキ）が初めて**（第62期・**採用しなかった**）。
