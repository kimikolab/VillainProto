...
現状値は README「肩代わりに出口を付けた —— 腹・還し（採用）と、まどろみ（棄却）」の
`spread` の表（第36期・42編成）。それより前の世代は「波に『ターン外の行動禁止』を置いたら、
読み手2体が割れた」（第27期・粛の後・38編成）、軛の節はその1つ前、渇きの節はさらに前、
「波の分離度を測る物差しを作った」の表は**いちばん前の値**。
**編成の行数が世代で違う**（38 → 42）ので、波を作り直すときは
...
**第27期の粛でこれが崩れた**——現状（第36期・42編成）は 100 / 63.4 / 72.9 / 58.0 / 53.6 で、
...
つもりで作ったが、速さ要件（書き手より遅い）と `Wounds = 1`（1ターンに傷1つ）が同居した結果、
**`傷/断ち` は実測で 1.00 に張り付いた**（`w ≥ 6` の発火は全セル 0.00 回/戦で、
軛 Cap=25 の予測も外れた）。`SetCounter(Wound, 0)` を書けば消費型になるわけではない
——**消費型かどうかは供給と消費の周期差で決まる。** 経緯は design/PHASE37_SEVER.md。
