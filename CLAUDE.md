...
現状値は **61編成**（第116期。**積み過ぎを据えのバンに載せた後**）で
...
所要は全編成でおおむね 10〜30秒、`bridge` だけ 30秒前後。
    ...
    dotnet run --project BattleSim -c Release 0 wound2 spill run    # 3版（S0 現行 / S1 `Dense` / S2 停止）× 73行 ＋ Q1 の再現（66 秒）
    dotnet run --project BattleSim -c Release 0 wound2 spill check  # 自己検査（必須4項目 ＋ (a') S0 が既定と一致 ／ (b) 経路が 0 ／ (c) 帳簿）
...
| 主題 | 期 |
|---|---|
| 傷軸（裂き・刻み・抉り・断ち・縫い・繕い・深手・巻き込み則） | 第 28, 37, 38, 39 / 73, 74, 75 / 84, 85, 86, 87, 89, 92, 93 / 104, 107, 108, **120, 121** 期 |
