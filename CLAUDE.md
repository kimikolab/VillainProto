...
**残り枠は 0。ロスターは第103期に 52 枚で確定した**（`UnitCatalog.All` は 52 体＝
ソラ・カリ・トメ・ヒヨ・**ソム**を含む。上限は 52 ＝ トランプ1組）。
**以後、新しい駒を足すなら既存の1枚を落とすことになる**——落とす候補の判定器具は
第82期の健康診断（`checkup`）にあるが、**あれは 51 体で回した表なのでソムを含めて回し直す必要がある。**
