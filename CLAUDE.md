...
**札は表示専用で、読んで分岐する規則は 0 件**（走査で定義31・出力1・doc参照1 のみ。
...
| 第131期 | 踏み込みの札（`UnitDef.Advances`）。**表示専用で盤面は1ビットも動かない**（読んで分岐する規則 0 件）。味方52＋敵40＋残置7 に1文から割り当て、**踏み込まないのは 味方22 / 敵11**。段0 で第128〜130期の則を取り込んだ |
