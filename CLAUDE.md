...
## 構成と絶対のルール
    ...
    docs/           BattleSim が吐く生成物 13ファイル（balance / units / chain / ablation /
                    pulse / engage / layout / reseat ／ **crossing** ／ **rules** ／ **watch** ／ **quality**
                    ／ **stock**）。手で編集しない。整合は `audit` で見る
...
現状値は **61編成**（**第134期も盤面を1ビットも動かしていない**ので数字は第130期のまま）で
...
| `design/LESSONS_101_.md` | 第101期〜 | 会戦の境界・手番という通貨・再行動・背かれ・尾灯・段（格上げ）。**いま盤面がどうなっているかを知るとき**。以後の期はここに追記する |
