...
**残り枠は 3**（第52期時点。`UnitCatalog.All` は 49 体＝ソラ・カリを含む。上限は 52）。
**うち1枠はオゴという素材が既にある**（第46期に棄却して残置しただけで `All` には載っていない）。
**第48期の「残り 5 / 分母 47」は古い**——第50期にソラ、第52期にカリが入った。

**通貨の厚みは駒で数える。CLAUDE.md の「毒 6書き/4読み」等は呼び出し箇所の数で、単位が違う。**
駒で数えると 毒 5/4・標 **2/1**（第52期にカリが2枚目の書き手）・痺 5(+敵3)/1・燃 1/1・IdleTurn 0/3・破片 3/1・傷 4/4・
...
**切れない駒は6体**（ある通貨の唯一の書き手か唯一の読み手）。
ヒサ⇔ザン（標）と ボルグ⇔ホタ（燃）は**相互唯一のペア**で、片方を切るともう片方も死ぬ。
残りはシガ（痺の唯一の読み手）とウロ（破片の唯一の読み手）。**寄与（`ablate`）が低くても切れない。**
...
7キーの内訳は 毒 6書き/4読み・標 **2/1**（第52期にカリ）・痺 7/1・燃 1/1・IdleTurn 3/2・**破片 2/0**・傷 4/4。
...
（第50期に一覧を固定した。第48・49期の残件B）。engine 側の窓口を持つ通貨は **9 / 10**（傷だけが持たない）:
    ...
    標    BattleEngine.cs:1152  SelectTargetChain。MarkPullPercent(75)% で主目標を標持ちへ差し替える
...
現状値は **54編成**（第52期。駆り立て2行を足した後）で 100 / 67.5 / 74.4 / 57.5 / **38.3**
...
**編成の行数が世代で違う**（38 → 42 → 45 → 48 → 50 → 52 → 54）ので、波を作り直すときは
