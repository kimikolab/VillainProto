...
（第50期に一覧を固定した。第48・49期の残件B）。engine 側の窓口を持つ通貨は **9 / 10**（傷だけが持たない）:
    ...
    燃    :203 / :229           TickStatuses が 6 削る ／ Ignite が残ターンを設定
          **滲み則はこの `Ignite` の中1箇所**——相手が傷を持つなら戻す先が 3 ではなく 4 になる
          （`BurnRules.Turns` は書き換えない）。**ただし供給者が毎ターン点け直すので紙の 2% しか払い出されない**
...
**味方の傷の読み手はハリ1枚だけ**で、しかも「敵と味方の深いほう」を自動で選ぶので選べない）。
...
**滲み則を採用した**（第90期。**傷を持つ相手には状態異常が深く入る**——毒は層 +1・燃焼は残ターン +1）。
`SoakRule.Default` = `true`。**新しい `TraitId` ゼロ・駒ゼロ・`CompareBuilds()` は1行も触っていない。**
...
第59期の「判定式の条件節が分母の何割を選ぶかを実装前に数える」を**拒否権の側でも**やること。
...
現状値は **61編成**（第74期。断ちの待ち方を V1 にした後）で
