...
**ただし交差帯（`Presets.Cross`）の `傷×被弾 (カド×ハリ×ノノ)` にはまだ生きている**（第109期で決める）。
...
1行も無い**（機構を測る第109期まで盤面に出さないため。**意図した状態**で、
...
**残る不整合**: `docs/units.md` からハリが消えたのに `docs/crossing.md` には残る。**第109期で決める。**
...
現状値は **61編成**（第108期。ハリ → トモの入れ替えと `刻み×澱み` への差し替えの後）で
...
`刻み×縫い` は入っていないので、**この期の差し替えは主判定の分母に1行も触れない**
（第107期の「主判定19行にナタを含む行が 0 行」と同じ形。**指示書の予測「歯止めが動く」は外れ**）。
全61行が第4波 −0.9 / 第5波 −1.0 動いたのは、差し替えた1行の低下を 61 で割ったぶん。
