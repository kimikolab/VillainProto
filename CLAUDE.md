...
——`BattleSim/Program.cs` は **68,000 行がトップレベル文**で、C# はそれを1つの `<Main>$` に畳むので
**全モードの局所変数が1つのスタックフレームに同居する。**
...
**いまはビルドのほうが測定より遅い**（第140期）。`BattleSim` の Release ビルドは **263〜294 秒**で、
`docs/` 全再生成（263 秒）と同じか、それより長い。
**Roslyn のピーク作業セットは 22.3 GB**——**これは HEAD 素の状態でも同じ**（22.3 対 22.1 GB）なので
特定の変更のせいではなく、**`BattleSim/Program.cs` の 68,000 行がトップレベル文で
1つの `<Main>$` に畳まれる**（第139期）という構造そのものの代金である。
**1回の変更あたりの待ちを次に縮めるなら、対象は測定ではなくビルド。**
