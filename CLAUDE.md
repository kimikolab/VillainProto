...
## 構成と絶対のルール
    ...
    DemoApp/        Godot 4 (C#) のデモ（第123期に取り込んだ）。sln には入っておらず単独ビルド。
                    ...
                    **道の上の隊・全滅した隊は触れない**／**死者は戻らない**（`CrossBoundary` が落とすので
                    席が空く）／**0 枚の隊は出せない**。**控えの駒は 6 枚**
                    （名指しの 2 枚 ＋ `ReserveRanking()` が規則で選ぶ 4 枚）。
...
**新しい則を足すとき**: `design/RULES_*.md` の末尾に次の ID（`R268` から）で本文を書き、
...
本体は `DemoApp/Map11.cs`（進行の規則）と `Map11Main.cs`（画面。判定は持たない）。
    ...
    Godot_console.exe --path DemoApp --headless -- --map11-phase0
        ...
        # 手書きの `Map11Info.TraitLines` に抜けがあれば**名指しで落ちる**（R260）
    ...
    Godot_console.exe --path DemoApp --headless -- --map11-phase172
        ...
        # 控えの駒 6 枚を**規則で選び直して**表にし、手書きの
        # `Map11Relations.Rules` の意味の1行に欠けがあれば**名指しで落ちる**（R260）。
...
| 表示と「見る」（`GodotApp` / `DemoApp` / 台本の窓口 / 盲点表 / 戦績パネル / 書き手 / 手番の外 / **遊べるマップ** / **情報の層** / **配置の図と封じ** / **拠点での組み直し**） | 第 97, 98, 99 / **123, 124, 125** / **169, 170, 171, 172** 期 |
