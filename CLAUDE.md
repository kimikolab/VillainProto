...
所要は全編成でおおむね 10〜30秒、`bridge` だけ 30秒前後。
    ...
    dotnet run --project BattleSim -c Release 0 funnel [モード] # 横流し（第62期→**第63期に測り直し**・どちらも**採用しなかった**）。V1 強化だけ / V3 強化と弱体 / 素体
    dotnet run --project BattleSim -c Release 0 funnel phase0    # 地図（席と並び / **席ごとの候補集合** / **床の判定** / 弱体の在庫 / `Dull` の先客 / 誹りと主判定）
    dotnet run --project BattleSim -c Release 0 funnel scan      # 弱体が厚い行の探索（61行・**味方側に落ちた弱体だけ**を `UnitTally.Dulled` で数える）
    dotnet run --project BattleSim -c Release 0 funnel why       # V3 の解剖（席ごとに「誰から取り上げて誰へ押し付けたか」＋ウツの被弾弱体の前後）
    dotnet run --project BattleSim -c Release 0 funnel alt       # 同じ表を別 seed 帯（0..199）で（第63期の主帯は 600..999）
...
**残り枠は 1**（第63期時点も変わらず。`UnitCatalog.All` は 51 体＝ソラ・カリ・トメ・ヒヨを含む。上限は 52）。
**その1枠に充てられる素材は3枚ある**——オゴ（第46期）・ゴウ（第49期）・**ヌキ（第62・63期に2度落ちた）**。
