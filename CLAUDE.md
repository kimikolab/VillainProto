...
**残り枠は 2**（第53期時点。`UnitCatalog.All` は 50 体＝ソラ・カリ・トメを含む。上限は 52）。
...
駒で数えると 毒 5/4・標 **3/2**（第52期にカリが2枚目の書き手／第53期にトメが2枚目の読み手）・痺 5(+敵3)/1・燃 1/1・IdleTurn 0/3・破片 3/1・傷 4/4・
...
現状値は **56編成**（第53期。止め2行を足した後）で 100 / 68.4 / 73.6 / 57.0 / **37.0**
...
**編成の行数が世代で違う**（38 → 42 → 45 → 48 → 50 → 52 → 54 → 56）ので、波を作り直すときは
...
- **攻撃力を上げる唯一の窓口は `BattleContext.Whet` の1箇所**（第56期）。
  **`AtkBonus` を直接足さないこと。** `Dull`（鈍らせる）の対義で、通るのは**他者強化の6経路だけ**
  ——駆り立て（カリ）／号令の鬨（ガン・開戦時）／号令の溜め（ガン・毎ターン）／縛め（クグ）／
  移り木（シオ）／**吐き戻し（ゴルム・engine 側）**。`Dull` も engine 側の1本（分かちのなまり）を
