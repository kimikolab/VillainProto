...
現状値は README「弱体化に戦闘中の供給を作った —— 突き返しのハネ」の `spread` の表
...
**編成の行数が世代で違う**（38 → 42 → 45）ので、波を作り直すときは
...
**第27期の粛でこれが崩れた**——現状（第41期・45編成）は 100 / 64.8 / 72.3 / 55.9 / 39.6 で、
**2番目の波が3番目より難しい**。**第五波の平均は歯止め 40.0 を 0.4pt 割っている**が、
これは行を1本足した組成効果で、波そのものは第40期の 40.5（44編成）から動いていない。
...
- **弱体化（`AtkBonus` を負にすること）の窓口は `BattleContext.Dull` の1箇所**（第42期）。
  **`AtkBonus` を直接引かないこと。** 渇きが `ctx.Heal` の入口に1つ立っているのと同じ形で、
  集約（引き受け・`TraitId.Bear`）の横取りがここに立っている。通るのは**5経路**
  （分かちのなまり／呪詛の敵側／呪詛の味方漏れ／突き返しのよろけ／萎縮）。
