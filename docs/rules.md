...
| 24 | `suture` | `SutureRule` | `SutureRule { Side = Both }` |  | `curse` / `hex` / `suture2` / `mender` / `gauge` / `gather` / `soak` / `hold2` / `cross` | 第85期〜（9 期） | ○ |
| 25 | `sutureFire` | `SutureFireRule` | `SutureFireRule { Fire = Swing }` | ○ | `hold2` | 第107期 |  |
| 26 | `spillWound` | `SpillWoundRule` | `SpillWoundRule { Enabled = True, Scope = All }` |  | `curse` / `hex` / `suture2` / `mender` / `gauge` / `gather` / `soak` / `cross` | 第85期〜（8 期） | ○ |
| 27 | `mend` | `MendRule` | `MendRule { Side = Wound }` |  | `curse` / `hex` / `mender` / `gauge` / `cross` | 第86期〜（9 期） | ○ |
...
| 34 | `encore` | `EncoreRule` | `EncoreRule { Enabled = True }` |  | `encore` / `tempo` | 第104期〜（2 期） | ○ |
| 35 | `rage` | `RageRule` | `RageRule { Mode = Amount, Gain = 3 }` |  | `tempo` / `hold` / `hold2` | 第106期〜（2 期） | ○ |
| 36 | `menderCost` | `MenderCostRule` | `MenderCostRule { Percent = 50 }` |  | `tempo` / `hold` | 第106期 | ○ |
| 37 | `loose` | `LooseRule` | `LooseRule { Shove = True }` |  | `tempo` / `hold` | 第106期 | ○ |
...
| `SutureFireRule` | `SutureFireRule { Fire = Swing }` | ○ | `hold2` | 第107期 |
| `SutureRule` | `SutureRule { Side = Both }` | ○ | `curse` / `hex` / `suture2` / `mender` / `gauge` / `gather` / `soak` / `hold2` / `cross` | 第85期〜（9 期） |
