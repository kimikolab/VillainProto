...
| 26 | `mend` | `MendRule` | `MendRule { Side = Wound }` |  | `curse` / `hex` / `mender` / `gauge` / `cross` | 第86期〜（8 期） | ○ |
...
| 28 | `gather` | `GatherRule` | `GatherRule { Enabled = True }` |  | `audit` / `derive` / `curse` / `hex` / `gather` / `deep` / `soak` / `cross` / `demo` | 第89期〜（8 期） | ○ |
| 29 | `soak` | `SoakRule` | `SoakRule { Poison = True, Burn = False, DullPerKind = 1 }` |  | `derive` / `curse` / `hex` / `soak` / `cross` | 第90期〜（8 期） | ○ |
| 30 | `deep` | `DeepRule` | `DeepRule { Enabled = False }` | ○ | `derive` / `curse` / `hex` / `deep` | 第93期〜（5 期） | ○ |
...
| `GatherRule` | `GatherRule { Enabled = True }` | ○ | `audit` / `derive` / `curse` / `hex` / `gather` / `deep` / `soak` / `cross` / `demo` | 第89期〜（8 期） |
...
| `ThornRule` | `ThornRule { Wound = None }` | ○ | `derive` / `curse` / `breadth` / `thorn` / `suture2` / `gauge` / `cross` | 第84期〜（12 期） |
